...
# Embedded file name: /usr/lib/python2.7/site-packages/twisted/internet/iocpreactor/tcp.py
# Compiled at: 2014-03-28 01:06:29
"""
...
connectExErrors = {ERROR_CONNECTION_REFUSED: (errno.WSAECONNREFUSED), 
...
class Connection(abstract.FileHandle, _SocketCloser, _AbortingMixin):
    ...
    """
    implements(IReadWriteHandle, interfaces.ITCPTransport, interfaces.ISystemHandle)
    TLS = False
    ...
    def writeToHandle(self, buff, evt):
        ...
        """
        return _iocp.send(self.getFileHandle(), buffer(buff, 0, self.SEND_LIMIT), evt)
...
class Client(_BaseBaseClient, _BaseTCPClient, Connection):
    ...
    def cbConnect(self, rc, bytes, evt):
        if rc:
            ...
            self.socket.setsockopt(socket.SOL_SOCKET, SO_UPDATE_CONNECT_CONTEXT, struct.pack('P', self.socket.fileno()))
...
class Port(_SocketCloser, _LogOwner):
    ...
    def handleAccept(self, rc, evt):
        ...
        if rc:
            ...
            protocol = self.factory.buildProtocol(self._addressType('TCP', rAddr[0], rAddr[1]))
            if protocol is None:
                evt.newskt.close()
            else:
                s = self.sessionno
                self.sessionno = s + 1
                transport = Server(evt.newskt, protocol, self._addressType('TCP', rAddr[0], rAddr[1]), self._addressType('TCP', lAddr[0], lAddr[1]), s, self.reactor)
