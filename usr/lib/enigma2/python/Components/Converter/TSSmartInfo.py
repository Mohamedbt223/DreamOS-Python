...
# Embedded file name: /usr/lib/enigma2/python/Components/Converter/TSSmartInfo.py
# Compiled at: 2017-03-25 12:41:55
...
DVB_S = iDVBFrontend.feSatellite
DVB_C = iDVBFrontend.feCable
DVB_T = iDVBFrontend.feTerrestrial

class TSSmartInfo(Poll, Converter, object):
    SMART_INFO_H = 1
    ...
    def getText(self):
        ...
        if not info:
            ...
            if self.type == self.SMART_INFO_H:
                ...
                if feinfo is None and xresol > 0:
                    prvd = ''
                    Ret_Text = 'IPTV'
                elif feinfo is not None and xresol > 0:
                    frontendData = feinfo and feinfo.getAll(True)
                    if frontendData is not None:
                        if frontendData.get('tuner_type') == DVB_S:
                        ...
                        elif frontendData.get('tuner_type') == DVB_C:
                            frequency = str(frontendData.get('frequency') / 1000) + ' MHz'
                            symbolrate = str(frontendData.get('symbol_rate') / 1000)
                            frontendDataReadable = ConvertToHumanReadable(frontendData, 'DVB-C')
                            modulation = frontendDataReadable['modulation']
                            tuner_type = frontendDataReadable['tuner_type']
                            Ret_Text = Ret_Text + frequency + '  ' + modulation + '  ' + symbolrate + '  ' + tuner_type
                        elif frontendData.get('tuner_type') == DVB_T:
                            frequency = str(frontendData.get('frequency') / 1000) + ' MHz'
                            frontendDataReadable = ConvertToHumanReadable(frontendData, 'DVB-T')
                            constellation = frontendDataReadable['constellation']
                            transmission_mode = frontendDataReadable['transmission_mode']
                            tuner_type = frontendDataReadable['tuner_type']
                            Ret_Text = Ret_Text + frequency + '  ' + constellation + '  ' + transmission_mode + '  ' + tuner_type
