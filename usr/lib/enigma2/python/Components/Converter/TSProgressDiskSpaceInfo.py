...
# Embedded file name: /usr/lib/enigma2/python/Components/Converter/TSProgressDiskSpaceInfo.py
# Compiled at: 2015-05-16 13:31:32
...
class TSProgressDiskSpaceInfo(Poll, Converter):
    HDDTEMP = 0
    LOADAVG = 1
    MEMTOTAL = 2
    MEMFREE = 3
    ...
    def __init__(self, type):
        ...
        Poll.__init__(self)
        type = type.split(',')
        self.percentFormat = 'Percent' in type
    ...
    def getValue(self):
        ...
        if self.type in (self.MEMTOTAL,
         ...
         self.SWAPTOTAL,
         self.SWAPFREE):
            entry = {(self.MEMTOTAL): 'Mem', (self.MEMFREE): 'Mem', (self.SWAPTOTAL): 'Swap', 
         ...
         self.HDDINFO,
         self.FLASHINFO):
            path = {(self.USBINFO): '/media/usb', (self.SDINFO): '/media/sd', (self.HDDINFO): '/media/hdd', 
    ...
    def getPathInfo(self):
        ...
        try:
            if path.exists(config.movielist.last_videodir.value):
                ...
                result[0] = stat.f_bsize * stat.f_blocks
                result[2] = (stat.f_bavail if stat.f_bavail != 0 else stat.f_bfree) * stat.f_bsize
                result[1] = stat.f_bsize * (stat.f_blocks - stat.f_bfree)
                result[3] = result[1] * 100 / result[0]
            return result
        except:
            return result
    ...
    def getSizeStr(self, value, u=0):
        ...
        if value >= 1024:
            ...
            fmt = '%(size)u %(unit)s'
        return fmt % {'size': value, 'frac': fractal, 
           'unit': (SIZE_UNITS[u])}
