...
# Embedded file name: /usr/lib/enigma2/python/Components/Converter/TSServiceInfo2.py
# Compiled at: 2013-01-31 21:58:45
...
class TSServiceInfo2(Converter, object):
    SERVICENAME = 0
    SERVICENUMBER = 1
    ORBITALPOSITION = 2
    SATNAME = 3
    ...
    def getText(self):
        ...
        if not info:
            return ''
        text = ''
        name = info.getName().replace('\x86', '').replace('\x87', '')
        ...
        for bouquet in bouquets:
            ...
            for channel in channels:
                if not channel[0].startswith('1:64:'):
                    list.append(channel[1].replace('\x86', '').replace('\x87', ''))
