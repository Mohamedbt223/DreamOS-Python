...
# Embedded file name: /usr/lib/enigma2/python/Components/Renderer/TSyWeatherPixmap.py
# Compiled at: 2015-01-25 12:33:58
from Renderer import Renderer
from enigma import ePixmap
from Components.AVSwitch import AVSwitch
from enigma import eEnv, ePicLoad, eRect, eSize, gPixmapPtr

class TSyWeatherPixmap(Renderer):
    ...
    def changed(self, what):
        if what[0] != self.CHANGED_CLEAR:
            if self.instance and not self.source.iconfilename == '':
                self.updateIcon(self.source.iconfilename)
                self.show()
            else:
                emptyIconFilename = '/usr/lib/enigma2/python/Plugins/TSimage/TSMediaPanel/weather_icons/empty.png'
                self.updateIcon(emptyIconFilename)
                self.hide()
        else:
