...
# Embedded file name: /usr/lib/enigma2/python/Plugins/Extensions/WebInterface/WebComponents/Sources/Movie.py
...
class Movie(Source):
    ...
    def __init__(self, session, movielist, func=LIST):
        ...
        self.res = (
         False, _('Missing or Wrong Argument'))
        return
    ...
    def moveMovie(self, param):
        ...
        if not param['sRef']:
            ...
            if service is not None:
                ...
                if not force and os.path.exists(destdir + basename):
                ...
                if basename.endswith('.ts'):
                    wasTs = True
                    basename = basename[:-3]
                elif basename.endswith(('.avi', '.mkv', '.mpg', '.mp4', '.m4v', '.flv',
                                        '.mov', '.iso', '.vob')):
                ...
                def moveFunc():
                    ...
                    errorlist = []
                    if wasTs:
                        suffixes = ('.ts.meta', '.ts.cuts', '.ts.ap', '.ts.sc', '.eit',
                                    '.ts', '.jpg')
                    else:
                        suffixes = (
                         '%s.ts.meta' % forcedSuffix, '%s.cuts' % forcedSuffix, forcedSuffix, '.jpg', '.eit')
                ...
                if background:
                    ...
                    return (
                     True, _("Moving Movie '%s' to '%s' in background.") % (name, destdir))
                errlist = moveFunc()
                if not errlist:
            ...
            return (
             result, _("Illegal Parameter Value: sRef - '%s'") % param['sRef'])
    ...
    def getMovieList(self):
        ...
        for serviceref, info, begin, unknown in self.movielist.list:
            ...
            servicename = ServiceReference(serviceref).getServiceName().replace('\x86', '').replace('\x87', '')
            append((
             serviceref.toString(),
             servicename,
    ...
    lut = {'ServiceReference': 0, 'Title': 1, 
       'Description': 2, 
       'Time': 3, 
       'TimeString': 4, 
       'Length': 5, 
