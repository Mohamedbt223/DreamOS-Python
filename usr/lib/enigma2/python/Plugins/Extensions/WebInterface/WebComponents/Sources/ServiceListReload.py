...
# Embedded file name: /usr/lib/enigma2/python/Plugins/Extensions/WebInterface/WebComponents/Sources/ServiceListReload.py
...
class ServiceListReload(Source):
    ...
    def getResult(self):
        if self.res:
            ...
            return (
             False, _('missing or wrong parameter mode [%i=both, %i=lamedb only, %i=userbouqets only, %i=transponders, %i=parentalcontrol white-/blacklist]') % (self.BOTH, self.LAMEDB, self.USERBOUQUETS, self.TRANSPONDERS, self.PARENTAL))
