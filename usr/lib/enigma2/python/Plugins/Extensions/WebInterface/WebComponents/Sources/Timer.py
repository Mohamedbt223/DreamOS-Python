...
# Embedded file name: /usr/lib/enigma2/python/Plugins/Extensions/WebInterface/WebComponents/Sources/Timer.py
...
class Timer(Source):
    ...
    def handleCommand(self, cmd):
        if self.func is self.ADDBYID:
            ...
            self.res = (
             False, _("Unknown function: '%s'") % self.func)
        return
    ...
    def delTimer(self, param):
        ...
        if 'sRef' in param:
            ...
            return (
             False, _('Missing Parameter: sRef'))
        if 'begin' in param:
            begin = int(float(param['begin']))
        else:
            return (
             False, _('Missing Parameter: begin'))
        if 'end' in param:
            end = int(float(param['end']))
        else:
            return (
             False, _('Missing Parameter: end'))
        try:
            for timer in self.recordtimer.timer_list + self.recordtimer.processed_timers:
                if str(timer.service_ref) == str(service_ref) and int(timer.begin) == begin and int(timer.end) == end:
                    ...
                    return (
                     True, _("The timer '%s' has been deleted successfully") % timer.name)

        except Exception:
            return (
             False, _('The timer has NOT been deleted'))
    ...
    def tvBrowser(self, param):
        ...
        if param['sRef'] is None:
            ...
            if param['command'] == 'del':
                del param['command']
                return self.delTimer(param)
            if param['command'] == 'change':
                del param['command']
                return self.editTimer(param)
    ...
    def recordNow(self, param):
        if not config.misc.recording_allowed.value:
            ...
            if param == 'undefinitely' or param == 'infinite':
                ret = (
                 True, 'Infinite Instant recording started')
                limitEvent = False
            else:
                ret = (
                 True, 'Instant record for current Event started')
            ...
            if event is not None:
                ...
                ret = (
                 False, 'No event found! Not recording!')
            if ret[0]:
    ...
    def editTimer(self, param):
        ...
        if 'sRef' not in param:
            ...
            if tm <= begin:
                ...
                if tm > begin and tm < end and repeated == 0:
                    pass
                elif repeated == 0:
                ...
                if delold:
                    if 'channelOld' in param and param['channelOld']:
                        ...
                        return (
                         False, _('Missing Parameter: channelOld'))
                                ...
                                if int(timer.begin) == beginOld:
                                    if int(timer.end) == endOld:
                                        ...
                                        if conflicts is None:
                                            self.recordtimer.timeChanged(timer)
                                            print '[WebComponents.Timer] editTimer: Timer changed!'
                                            return (
                                             True, "Timer '%s' changed" % timer.name)
                    ...
                    return (
                     False, _("Could not find timer '%s' with given start and end time!") % name)
                try:
    ...
    def writeTimerList(self, force=False):
        if config.plugins.Webinterface.autowritetimer.value or force:
            ...
            return (
             True, _('TimerList has been saved'))
        else:
            return (
             False, _('TimerList has not been saved'))
