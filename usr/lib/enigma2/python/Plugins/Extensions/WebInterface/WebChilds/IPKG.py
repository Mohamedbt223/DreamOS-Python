...
# Embedded file name: /usr/lib/enigma2/python/Plugins/Extensions/WebInterface/WebChilds/IPKG.py
...
class IPKGResource(resource.Resource):
        ...
        if config.plugins.Webinterface.anti_hijack.value and len(request.args) > 0 and request.method == 'GET':
            ...
            if self.command is not None:
                if self.command in IPKGResource.SIMPLECMDS:
                    ...
                    if self.command in IPKGResource.PACKAGECMDS:
    ...
    def buildCmd(self, parms=[]):
        cmd = [
         IPKGResource.IPKG_PATH, basename(IPKGResource.IPKG_PATH), self.command] + parms
