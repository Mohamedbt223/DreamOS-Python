...
# Embedded file name: /usr/lib/enigma2/python/Plugins/Extensions/WebInterface/WebSocket/DreamboxServerProtocol.py
...
class DreamboxServerProtocol(WebSocketServerProtocol):
    ...
    KEY_EPG_NOW = 0
    KEY_EPG_NEXT = 1
    REQUEST_BASE_SCHEMA = vol.Schema({(vol.Required('id')): int, 
       (vol.Optional('cookie')): unicode})
    BASE_MESSAGE_SCHEMA = REQUEST_BASE_SCHEMA.extend({(vol.Required('type')): (vol.Any(TYPE_AUTH, TYPE_GET_SERVICES, TYPE_GET_EPG_NOWNEXT, TYPE_GET_STREAM_SETTINGS, TYPE_PING))}, extra=vol.ALLOW_EXTRA)
    AUTH_MESSAGE_SCHEMA = REQUEST_BASE_SCHEMA.extend({(vol.Required('type')): TYPE_AUTH, 
       (vol.Exclusive('token', 'auth')): unicode, 
       (vol.Exclusive('session', 'auth')): unicode})
    GET_SERVICES_MESSAGE_SCHEMA = REQUEST_BASE_SCHEMA.extend({(vol.Required('type')): TYPE_GET_SERVICES, 
       (vol.Required('reference')): unicode})
    GET_EPG_NOWNEXT_MESSAGE_SCHEMA = REQUEST_BASE_SCHEMA.extend({(vol.Required('type')): TYPE_GET_EPG_NOWNEXT, 
       (vol.Required('reference')): unicode, 
       (vol.Required('which')): (vol.Any(KEY_EPG_NOWNEXT, KEY_EPG_NOW, KEY_EPG_NEXT))})
    GET_STREAM_SETTINGS_SCHEMA = REQUEST_BASE_SCHEMA.extend({(vol.Required('type')): TYPE_GET_STREAM_SETTINGS})
    server = None
    session = None
    ...
    def do_get_services(self, msg):
        ...
        for service in services:
            svcs.append({'reference': (service[0]), 
               'name': (service[1])})
    ...
    def do_get_epg_nownext(self, msg):
        ...
        def evtToDict(event):
            return {'id': (event[0]), 
               ...
               'current': (event[3]), 
               'title': (event[4]), 
               'short_description': (event[5]), 
               'extended_description': (event[6]), 
               'reference': (event[7]), 
               'name': (event[8])}
    ...
    def do_get_stream_settings(self, msg):
        ...
        if not msg:
            return
        id = msg['id']
        result = {'source': (streamServerControl.config.streamserver.source.value), 
