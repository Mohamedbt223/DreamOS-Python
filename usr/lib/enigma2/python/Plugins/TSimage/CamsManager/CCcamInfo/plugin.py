...
# Embedded file name: /usr/lib/enigma2/python/Plugins/TSimage/CamsManager/CCcamInfo/plugin.py
# Compiled at: 2016-11-22 07:49:52
...
from enigma import getDesktop, eListboxPythonMultiContent, ePoint, eTimer, getDesktop, gFont, iPlayableService, iServiceInformation, loadPNG, RT_HALIGN_RIGHT
...
class EcmInfoScreen(Screen):
    ...
    def __init__(self, session):
        ...
        self.systemCaids = {'06': 'irdeto', '01': 'seca', 
           '18': 'nagra', 
           '05': 'via', 
           '0B': 'conax', 
           '17': 'betacrypt', 
...
class EcmInfoPositioner(Screen):
    ...
    def __init__(self, session):
        ...
        self['ecmInfo'] = Label('')
        self['actions'] = ActionMap(['CCcamInfoActions'], {'left': (self.left), 'up': (self.up), 
           'right': (self.right), 
           'down': (self.down), 
           'ok': (self.ok), 
           'cancel': (self.exit)}, -1)
...
class EcmInfoConfigMenu(ConfigListScreen, Screen):
    skin_1920 = '\n\t<screen position="center,center" size="840,270" title="CCcam Info">\n\t\t<widget name="config" position="15,15" size="810,240" scrollbarMode="showOnDemand" />\n\t</screen>'
    ...
    def __init__(self, session):
        ...
        ConfigListScreen.__init__(self, [getConfigListEntry(_('Show ecm.info:'), config.plugins.CCcamInfo.ecmInfoEnabled),
...
def Plugins(**kwargs):
    return [
     PluginDescriptor(name='CCcam Info %s' % VERSION, description=_('This plugin shows you the status of your CCcam'), where=[PluginDescriptor.WHERE_EXTENSIONSMENU, PluginDescriptor.WHERE_PLUGINMENU], icon='CCcamInfo.png', fnc=main), PluginDescriptor(where=[PluginDescriptor.WHERE_SESSIONSTART], fnc=sessionstart), PluginDescriptor(name='Ecm Info', where=PluginDescriptor.WHERE_MENU, fnc=startEcmInfoConfig)]
...
def CCcamListEntry(name, idx):
    ...
    if idx == 10:
        ...
        idx = 'info'
    skin_path = resolveFilename(SCOPE_CURRENT_SKIN) + '/skin_default/buttons'
    if fileExists(skin_path + '/key_%s.png' % str(idx)):
        png = skin_path + '/key_%s.png' % str(idx)
    else:
    ...
    if desktopSize.width() == 1920:
        ...
        txtsizeY = 25
    if fileExists(png):
        res.append(MultiContentEntryPixmapAlphaBlend(pos=(pixposX, pixposY), size=(pixsizeX, pixsizeY), png=loadPNG(png)))
    res.append(MultiContentEntryText(pos=(txtposX, txtposY), size=(txtsizeX, txtsizeY), font=0, text=name))
    return res
...
def CCcamServerListEntry(name, color):
    res = [
     name]
    skin_path = resolveFilename(SCOPE_CURRENT_SKIN) + '/skin_default/buttons'
    if fileExists(skin_path + '/%s.png' % color):
        png = skin_path + '/%s.png' % color
    else:
        png = '/usr/lib/enigma2/python/Plugins/TSimage/TSimagePanel/buttons/%s.png' % color
    if desktopSize.width() == 1920:
        pixposX = 8
        pixposY = 6
        pixsizeX = 24
...
def CCcamShareListEntry(hostname, type, caid, system, uphops, maxdown):
    res = [
     (
      ...
      maxdown)]
    if desktopSize.width() == 1920:
        res.append(MultiContentEntryText(pos=(8, 0), size=(375, 35), font=0, text=hostname))
        res.append(MultiContentEntryText(pos=(375, 0), size=(375, 35), font=0, text=_('Type: ') + type, flags=RT_HALIGN_RIGHT))
        res.append(MultiContentEntryText(pos=(8, 30), size=(375, 35), font=0, text=_('CaID: ') + caid))
        res.append(MultiContentEntryText(pos=(375, 30), size=(375, 35), font=0, text=_('System: ') + system, flags=RT_HALIGN_RIGHT))
        res.append(MultiContentEntryText(pos=(8, 60), size=(375, 35), font=0, text=_('Uphops: ') + uphops))
        res.append(MultiContentEntryText(pos=(375, 60), size=(375, 35), font=0, text=_('Maxdown: ') + maxdown, flags=RT_HALIGN_RIGHT))
    else:
        res.append(MultiContentEntryText(pos=(5, 0), size=(250, 20), font=0, text=hostname))
        res.append(MultiContentEntryText(pos=(250, 0), size=(250, 20), font=0, text=_('Type: ') + type, flags=RT_HALIGN_RIGHT))
        res.append(MultiContentEntryText(pos=(5, 20), size=(250, 20), font=0, text=_('CaID: ') + caid))
        res.append(MultiContentEntryText(pos=(250, 20), size=(250, 20), font=0, text=_('System: ') + system, flags=RT_HALIGN_RIGHT))
        res.append(MultiContentEntryText(pos=(5, 40), size=(250, 20), font=0, text=_('Uphops: ') + uphops))
...
def CCcamShareViewListEntry(caidprovider, providername, numberofcards, numberofreshare):
    ...
    if desktopSize.width() == 1920:
        res.append(MultiContentEntryText(pos=(8, 0), size=(645, 35), font=0, text=providername))
        res.append(MultiContentEntryText(pos=(645, 0), size=(75, 35), font=0, text=numberofcards, flags=RT_HALIGN_RIGHT))
        res.append(MultiContentEntryText(pos=(720, 0), size=(75, 35), font=0, text=numberofreshare, flags=RT_HALIGN_RIGHT))
    else:
        res.append(MultiContentEntryText(pos=(5, 0), size=(430, 20), font=0, text=providername))
...
def CCcamConfigListEntry(file):
    ...
    if content == org:
        ...
        png = lock_off
    if desktopSize.width() == 1920:
        res.append(MultiContentEntryPixmapAlphaBlend(pos=(4, 0), size=(36, 36), png=png))
        res.append(MultiContentEntryText(pos=(52, 0), size=(825, 40), font=0, text=name))
    else:
...
class CCcamInfoMain(Screen):
    skin_1920 = '\n\t<screen position="center,center" size="750,640" title="CCcam Info" >\n\t\t<widget name="menu" position="0,0" size="750,640" scrollbarMode="showOnDemand" />\n\t</screen>'
    ...
    def __init__(self, session):
        ...
        self['actions'] = NumberActionMap(['CCcamInfoActions'], {'1': (self.keyNumberGlobal), '2': (self.keyNumberGlobal), 
...
class CCcamInfoEcmInfoSelection(Screen):
    skin_1920 = '\n\t<screen position="center,center" size="750,630" title="CCcam Info" >\n\t\t<widget name="list" position="0,0" size="750,630" scrollbarMode="showOnDemand" />\n\t</screen>'
...
class CCcamInfoInfoScreen(Screen):
    skin_1920 = '\n\t<screen position="center,center" size="750,630" title="CCcam Info" >\n\t\t<widget name="text" position="0,0" size="750,630" font="Regular;22" />\n\t</screen>'
    skin_1280 = '\n\t<screen position="center,center" size="500,420" title="CCcam Info" >\n\t\t<widget name="text" position="0,0" size="500,420" font="Regular;33" />\n\t</screen>'
    if desktopSize.width() == 1920:
        skin = skin_1920
    else:
        skin = skin_1280
    ...
    def __init__(self, session, info):
        ...
        self['actions'] = ActionMap(['CCcamInfoActions'], {'ok': (self.close), 'cancel': (self.close), 
           'up': (self['text'].pageUp), 
...
class CCcamShareViewMenu(Screen, HelpableScreen):
    skin_1920 = '\n\t<screen position="center,center" size="840,645" title="CCcam Info" >\n\t\t<widget name="list" position="0,0" size="840,480" scrollbarMode="showOnDemand" />\n\t\t<eLabel position="15,483" size="810,2" backgroundColor="#ffffff" />\n\t\t<widget name="uphops" position="15,510" size="390,38" font="Regular;30" />\n\t\t<widget name="cards" position="435,510" size="390,38" halign="right" font="Regular;30" />\n\t\t<widget name="providers" position="15,555" size="390,38" font="Regular;30" />\n\t\t<widget name="reshare" position="435,555" size="390,38" halign="right" font="Regular;30" />\n\t\t<widget name="title" position="0,600" size="840,30" halign="center" font="Regular;30" />\t\n\t</screen>'
    skin_1280 = '\n\t<screen position="center,center" size="560,430" title="CCcam Info" >\n\t\t<widget name="list" position="0,0" size="560,320" scrollbarMode="showOnDemand" />\n\t\t<eLabel position="10,322" size="540,1" backgroundColor="#ffffff" />\n\t\t<widget name="uphops" position="10,340" size="260,25" font="Regular;20" />\n\t\t<widget name="cards" position="290,340" size="260,25" halign="right" font="Regular;20" />\n\t\t<widget name="providers" position="10,370" size="260,25" font="Regular;20" />\n\t\t<widget name="reshare" position="290,370" size="260,25" halign="right" font="Regular;20" />\n\t\t<widget name="title" position="0,400" size="560,20" halign="center" font="Regular;20" />\t\n\t</screen>'
    if desktopSize.width() == 1920:
        skin = skin_1920
    else:
        skin = skin_1280
    ...
    def __init__(self, session, url):
        ...
        self['reshare'] = Label()
        self['title'] = Label()
        self['actions'] = HelpableNumberActionMap(self, 'CCcamInfoActions', {'cancel': (self.exit, _('close share view')), '0': (
               self.getUphop, _('show cards with uphop 0')), 
           '1': (
               self.getUphop, _('show cards with uphop 1')), 
           '2': (
               self.getUphop, _('show cards with uphop 2')), 
           '3': (
               self.getUphop, _('show cards with uphop 3')), 
           '4': (
               self.getUphop, _('show cards with uphop 4')), 
           '5': (
               self.getUphop, _('show cards with uphop 5')), 
           '6': (
               self.getUphop, _('show cards with uphop 6')), 
           '7': (
               self.getUphop, _('show cards with uphop 7')), 
           '8': (
               self.getUphop, _('show cards with uphop 8')), 
           '9': (
               self.getUphop, _('show cards with uphop 9')), 
           'green': (
                   self.showAll, _('show all cards')), 
           'incUphop': (
                      self.incUphop, _('increase uphop by 1')), 
           'decUphop': (
                      self.decUphop, _('decrease uphop by 1')), 
           'ok': (
                self.getServer, _("get the cards' server"))}, -1)
...
class CCcamInfoSubMenu(Screen):
    skin_1920 = '\n\t<screen position="center,center" size="750,630" title="CCcam Info" >\n\t\t<widget name="list" position="15,15" size="750,400" scrollbarMode="showOnDemand" />\n\t\t<eLabel position="5,423" size="730,2" backgroundColor="#ffffff" />\n\t\t<widget name="info" position="15,428" size="750,248" font="Regular;24" transparent="1" />\n\t</screen>'
    skin_1280 = '\n\t<screen position="center,center" size="500,420" title="CCcam Info" >\n\t\t<widget name="list" position="0,0" size="500,250" scrollbarMode="showOnDemand" />\n\t\t<eLabel position="10,252" size="480,1" backgroundColor="#ffffff" />\n\t\t<widget name="info" position="10,255" size="500,165" font="Regular;16" transparent="1" />\n\t</screen>'
    if desktopSize.width() == 1920:
        skin = skin_1920
    else:
        skin = skin_1280
    ...
    def __init__(self, session, infoList, url):
        ...
        for x in self.infoList:
            if x[5].replace(_('Connected: '), '') == '':
                list.append(CCcamServerListEntry(x[0], 'grey'))
            elif x[1] == _('Cards: 0'):
...
class CCcamInfoConfigMenu(ConfigListScreen, Screen):
    skin_1920 = '\n\t<screen position="center,center" size="840,240" title="CCcam Info">\n\t\t<widget name="config" position="0,0" size="840,240" scrollbarMode="showOnDemand" />\n\t</screen>'
    ...
    def __init__(self, session, profile):
        ...
        ConfigListScreen.__init__(self, [getConfigListEntry(_('Name:'), config.plugins.CCcamInfo.name),
...
class CCcamInfoRemoteBoxMenu(Screen):
    skin_1920 = '\n\t<screen position="center,center" size="840,630" title="CCcam Info" >\n\t\t<ePixmap pixmap="skin_default/buttons/red.png" position="0,0" size="210,60" transparent="1" alphatest="on" />\n\t\t<ePixmap pixmap="skin_default/buttons/green.png" position="210,0" size="210,60" transparent="1" alphatest="on" />\n\t\t<ePixmap pixmap="skin_default/buttons/yellow.png" position="420,0" size="210,60" transparent="1" alphatest="on" />\n\t\t<ePixmap pixmap="skin_default/buttons/blue.png" position="630,0" size="210,60" transparent="1" alphatest="on" />\n\t\t<widget name="key_red" position="0,0" zPosition="1" size="210,60" font="Regular;30" valign="center" halign="center" backgroundColor="#1f771f" transparent="1" />\n\t\t<widget name="key_green" position="210,0" zPosition="1" size="210,60" font="Regular;30" valign="center" halign="center" backgroundColor="#1f771f" transparent="1" />\n\t\t<widget name="key_yellow" position="420,0" zPosition="1" size="210,60" font="Regular;30" valign="center" halign="center" backgroundColor="#1f771f" transparent="1" />\n\t\t<widget name="key_blue" position="630,0" zPosition="1" size="210,60" font="Regular;30" valign="center" halign="center" backgroundColor="#1f771f" transparent="1" />\n\t\t<widget name="list" position="0,75" size="840,540" scrollbarMode="showOnDemand" />\n\t</screen>'
    ...
    def __init__(self, session):
        ...
        self['list'] = MenuList([])
        self['actions'] = ActionMap(['CCcamInfoActions'], {'cancel': (self.exit), 'ok': (self.profileSelected), 
           'red': (self.delete), 
           'green': (self.new), 
...
class CCcamInfoShareInfo(Screen):
    ...
    def __init__(self, session, hostname, url):
        ...
        self['actions'] = ActionMap(['CCcamInfoActions'], {'cancel': (self.exit), 'red': (self.uhopsPlus), 
...
class CCcamInfoConfigSwitcher(Screen):
    ...
    def __init__(self, session):
        ...
        self['actions'] = ActionMap(['CCcamInfoActions'], {'ok': (self.activate), 'cancel': (self.close), 
           'red': (self.delete), 
           'green': (self.activate), 
           'yellow': (self.rename), 
...
class CCcamInfoMenuConfig(Screen):
    skin_1920 = '\n\t<screen position="center,center" size="840,630" title="CCcam Info" >\n\t\t<ePixmap pixmap="skin_default/buttons/red.png" position="0,0" size="210,60" transparent="1" alphatest="on" />\n\t\t<ePixmap pixmap="skin_default/buttons/green.png" position="210,0" size="210,60" transparent="1" alphatest="on" />\n\t\t<ePixmap pixmap="skin_default/buttons/yellow.png" position="420,0" size="210,60" transparent="1" alphatest="on" />\n\t\t<ePixmap pixmap="skin_default/buttons/blue.png" position="630,0" size="210,60" transparent="1" alphatest="on" />\n\t\t<widget name="key_red" position="0,0" zPosition="1" size="210,60" font="Regular;30" valign="center" halign="center" backgroundColor="#1f771f" transparent="1" />\n\t\t<widget name="key_green" position="210,0" zPosition="1" size="210,60" font="Regular;30" valign="center" halign="center" backgroundColor="#1f771f" transparent="1" />\n\t\t<widget name="key_yellow" position="420,0" zPosition="1" size="210,60" font="Regular;30" valign="center" halign="center" backgroundColor="#1f771f" transparent="1" />\n\t\t<widget name="list" position="0,75" size="840,520" scrollbarMode="showOnDemand" />\n\t</screen>'
    skin_1280 = '\n\t<screen position="center,center" size="560,420" title="CCcam Info" >\n\t\t<ePixmap pixmap="skin_default/buttons/red.png" position="0,0" size="140,40" transparent="1" alphatest="on" />\n\t\t<ePixmap pixmap="skin_default/buttons/green.png" position="140,0" size="140,40" transparent="1" alphatest="on" />\n\t\t<ePixmap pixmap="skin_default/buttons/yellow.png" position="280,0" size="140,40" transparent="1" alphatest="on" />\n\t\t<ePixmap pixmap="skin_default/buttons/blue.png" position="420,0" size="140,40" transparent="1" alphatest="on" />\n\t\t<widget name="key_red" position="0,0" zPosition="1" size="140,40" font="Regular;20" valign="center" halign="center" backgroundColor="#1f771f" transparent="1" />\n\t\t<widget name="key_green" position="140,0" zPosition="1" size="140,40" font="Regular;20" valign="center" halign="center" backgroundColor="#1f771f" transparent="1" />\n\t\t<widget name="key_yellow" position="280,0" zPosition="1" size="140,40" font="Regular;20" valign="center" halign="center" backgroundColor="#1f771f" transparent="1" />\n\t\t<widget name="list" position="0,50" size="560,360" scrollbarMode="showOnDemand" />\n\t</screen>'
    if desktopSize.width() == 1920:
        skin = skin_1920
    else:
    ...
    def __init__(self, session):
        ...
        self['actions'] = ActionMap(['CCcamInfoActions'], {'ok': (self.changeState), 'cancel': (self.close), 
           'red': (self.close), 
           'green': (self.save), 
