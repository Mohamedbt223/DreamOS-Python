...
# Embedded file name: /usr/lib/enigma2/python/Plugins/TSimage/TSimagePanel/Stools/TSsatEditor/ServiceEditor/Lamedb.py
# Compiled at: 2012-10-20 12:04:17
import thread
...
class Lamedb():
    ...
    def translateService(self, serviceData):
        ...
        if serviceData is None:
            ...
            name = serviceData[1].strip().replace('\x86', '').replace('\x87', '')
            service.update({'name': name})
            provider_data = serviceData[2].strip().split(',')
            for y in provider_data:
                raw = y.split(':')
                if raw[0] == 'p':
                    service['provider'] = raw[1].strip().replace('\x86', '').replace('\x87', '')
                elif raw[0] == 'c':
                    cacheIDs = service.get('cacheIDs', None)
                    if cacheIDs is None:
                        service['cacheIDs'] = [
