...
# Embedded file name: /usr/lib/enigma2/python/Plugins/TSimage/TSimagePanel/Stools/download.py
# Compiled at: 2015-12-25 18:03:52
...
class TSilangGroups(Screen):
    ...
    def __init__(self, session):
        ...
        self.yellowStatus = LoadPixmap(cached=True, path=resolveFilename(SCOPE_CURRENT_PLUGIN, '/usr/lib/enigma2/python/Plugins/TSimage/TSimagePanel/buttons/blue.png'))
    ...
    def downloadxmlpage(self, result=True):
        if not os_path.exists('/tmp/.dpkg_ready'):
            ...
            self.dpkglist_filename = '/tmp/.tmplist'
            cmd = "cat /tmp/.dpkglist | grep 'enigma2-language-' > /tmp/.tmplist"
            os_system(cmd)
            self._gotPageLoad()
        return
    ...
    def _gotPageLoad(self):
        count = 0
        self.ipkList = []
        fm = open(self.dpkglist_filename)
        line = 'dummy'
        sp = []
        while line:
            line = fm.readline()
