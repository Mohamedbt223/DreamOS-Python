...
config.skin.primary_skin = ConfigText(default = "ts-MetrixFHD/skin.xml")
...
try:
	hasInclude = False
	modulesPath = (resolveFilename(SCOPE_SKIN, config.skin.primary_skin.value))[:-8] + "modules"
	for file in listdir(modulesPath):
		if file.endswith(".xml"):
			filename = "/".join([modulesPath,file])
			hasInclude = True
			dom_skins.append((modulesPath, xml.etree.cElementTree.parse(filename).getroot()))
	if hasInclude:
		profile("LoadInclude")
except:
	print "loading include failed"
#patch <<<
...
def loadSingleSkinData(desktop, skin, path_prefix):
	...
	# patch load include tag
        for c in skin.findall("include"):
...
# patch lookup panels
def lookupPanels(skin, path_prefix):
	counter = 0
	for screenpanel in skin.findall("screenpanel"):
		name = screenpanel.attrib.get("name")
		if name not in panelNames:
			panelNames[name] = screenpanel
			counter += 1
		else:
			print "panel name %s already exists, not added" %(name)
# end patch
...
def parseWidgets(name, node, screen, skin_path_prefix, visited_components, group=None):
	for widget in node.getchildren():
		...
		# patch screenpanel
		if w_tag == "panel":
			pname = widget.attrib.get('name')
			pwidget = panelNames.get(pname)
			if pwidget:
				parseWidgets(name, pwidget, screen, skin_path_prefix, visited_components)
			else:
				print "The panel %s refers to a screenpanel not defined" %(pname)
			continue
		# end patch
