...
class ChannelContextMenu(Screen):
    ...
    def __init__(self, session, csel):
        ...
        if csel.bouquet_mark_edit == OFF and not csel.movemode:
            if not inBouquetRootList:
                ...
                if isPlayable:
                    if config.ParentalControl.configured.value:
                        ...
                        if parentalControl.getProtectionLevel(csel.getCurrentSelection().toCompareString()) == -1:
                            append_when_current_valid(current, menu, (_("add to parental protection"),
                                                                      boundFunction(self.addParentalProtection,
                                                                                    csel.getCurrentSelection())),
                                                      level=0)
                        else:
                            append_when_current_valid(current, menu, (_("remove from parental protection"),
                                                                      boundFunction(self.removeParentalProtection,
                                                                                    csel.getCurrentSelection())),
                                                      level=0)
                    if haveBouquets:
                        ...
                        if not inBouquet or bouquetCnt > 1:
                            append_when_current_valid(current, menu,
                                                      (_("add service to bouquet"), self.addServiceToBouquetSelected),
                                                      level=0)
                    else:
                        if not inBouquet:
                            append_when_current_valid(current, menu,
                                                      (_("add service to favourites"),
                                                       self.addServiceToBouquetSelected),
                                                      level=0)
                else:
                    # --- Satellites-only tools (including our style toggle) ---
                    if current_root_path and current_root_path.find('FROM SATELLITES') != -1:
                        # ensure the flag exists on the selection (session-only)
                        if not hasattr(self.csel, 'sat_list_single'):
                            self.csel.sat_list_single = False

                        # style toggle entry
                        if self.csel.sat_list_single:
                            menu.append(ChoiceEntryComponent(text=(_("Satellite [Single folder]"), self._setSatStyleDefault)))
                        else:
                            menu.append(ChoiceEntryComponent(text=(_("Satellite  [Default]"), self._setSatStyleSingle)))

                        unsigned_orbpos = current.getUnsignedData(4) >> 16
                        print "unsigned orbpos %08x" % unsigned_orbpos
                        if unsigned_orbpos == 0xFFFF:  # Cable
                            append_when_current_valid(current, menu,
                                                      (_("remove cable services"), self.removeSatelliteServices),
                                                      level=0)
                        elif unsigned_orbpos == 0xEEEE:  # Terrestrial
                            append_when_current_valid(current, menu,
                                                      (_("remove terrestrial services"), self.removeSatelliteServices),
                                                      level=0)
                        else:
                            append_when_current_valid(current, menu,
                                                      (_("remove selected satellite"), self.removeSatelliteServices),
                                                      level=0)

                    if haveBouquets:
                        if not inBouquet and current_sel_path.find("PROVIDERS") == -1:
                            append_when_current_valid(current, menu,
                                                      (_("copy to bouquets"), self.copyCurrentToBouquetList),
                                                      level=0)
                    if current_sel_path.find("flags == %d" % (FLAG_SERVICE_NEW_FOUND)) != -1:
                        append_when_current_valid(current, menu,
                                                  (_("remove all new found flags"), self.removeAllNewFoundFlags),
                                                  level=0)
                ...
                if isPlayable and SystemInfo.get("NumVideoDecoders", 1) > 1 and not isinstance(self.csel,
                                                                                               ChannelSelectionRadio):
                    append_when_current_valid(current, menu, (_("Activate Picture in Picture"), self.showServiceInPiP),
                                              level=0, key="blue")
                ...
                if not csel.movemode:
                    ...
                    if not inBouquetRootList and current_root and not (current_root.flags & eServiceReference.isGroup):
                        ...
                        if haveBouquets:
                            ...
                            append_when_current_valid(current, menu,
                                                      (_("enable favourite edit"), self.bouquetMarkStart), level=0)
                        if current_sel_flags & eServiceReference.isGroup:
                            append_when_current_valid(current, menu,
                                                      (_("edit alternatives"), self.editAlternativeServices), level=2)
                            append_when_current_valid(current, menu,
                                                      (_("show alternatives"), self.showAlternativeServices), level=2)
                            append_when_current_valid(current, menu,
                                                      (_("remove all alternatives"), self.removeAlternativeServices),
                                                      level=2)
                        elif not current_sel_flags & eServiceReference.isMarker:
                            append_when_current_valid(current, menu,
                                                      (_("add alternatives"), self.addAlternativeServices), level=2)
                else:
    ...
    def _setSatStyleSingle(self):
        config.servicelist.sat_list_single.value = True
        config.servicelist.sat_list_single.save()
        self.csel.sat_list_single = True
        try:
            self.csel.showSatellites()
        except:
            pass
        self.close()

    def _setSatStyleDefault(self):
        config.servicelist.sat_list_single.value = False
        config.servicelist.sat_list_single.save()
        self.csel.sat_list_single = False
        try:
            self.csel.showSatellites()
...
class ChannelSelectionBase(Screen):
    ...
    def __init__(self, session):
        ...
        self.mode = MODE_TV
        self.sat_list_single = config.servicelist.sat_list_single.value
    ...
    def showSatellites(self):
        ...
        # session-only flag (no Config)
        style_single = getattr(self, 'sat_list_single', False)

        refstr = '%s FROM SATELLITES ORDER BY satellitePosition' % (self.service_types)
        if self.preEnterPath(refstr):
            return
        ref = eServiceReference(refstr)
        justSet = False
        prev = None
        ...
        if style_single:
            # ---- One folder per satellite ----
            seen_orbpos = set()
            while True:
                service = servicelist.getNext()
                if not service.valid():
                    break
                if not (service.flags & eServiceReference.isDirectory):
                    continue

                path = service.getPath() or ""
                # Skip alternate views
                if ("FROM PROVIDER" in path) or ("flags ==" in path) or ("numCAIDs" in path):
                    continue
                ...
                if unsigned_orbpos in seen_orbpos:
                    continue
                seen_orbpos.add(unsigned_orbpos)
                ...
                try:
                    ...
                    if unsigned_orbpos == 0xFFFF:
                        ...
                        if orbpos > 1800:
                            orbpos_lbl = 3600 - orbpos;
                            h = _("W")
                        else:
                            orbpos_lbl = orbpos;
                            h = _("E")
                        service_name = ("%d.%d" + h) % (orbpos_lbl / 10, orbpos_lbl % 10)
            ...
            while True:
                ...
                path = service.getPath() or ""
                if path.find("FROM PROVIDER") != -1:
                    service_type = _("Providers")
                elif path.find("flags == %d" % (FLAG_SERVICE_NEW_FOUND)) != -1:
                    service_type = _("New")
                elif path.find("numCAIDs") != -1:
        ...
        # Keep your “Current Transponder” extra entry
        cur_ref = self.session.nav.getCurrentlyPlayingServiceReference()
        if cur_ref:
            pos = self.service_types.rfind(':')
            refstr = '%s (channelID == %08x%04x%04x) && %s ORDER BY name' % (
            ...
            cur = eServiceReference(refstr)
            cur.flags |= eServiceReference.sort1
            cur.setName(_("Current Transponder"))
            self.servicelist.addService(cur, True)
            self.servicelist.moveUp()
...
config.servicelist.sat_list_single = ConfigBoolean(default=False)
